...
function updateSummaryResults(results) {
    ...
    // --- Gather original circling minima ---
    CATS.forEach(cat=>{
    ...
    blocks.forEach(proc => {
        ...
        CATS.forEach(cat => {
            ...
            // Check if circling needs raising
    ...
    // -------- Render summary (only results section, no STATE/STANDARD toggle) --------
    let html = `<h2>Summary of Results</h2>`;

    // Display each selected procedure
    blocks.forEach(proc => {
        if (!results[proc.code]) return;
        html += `<div style="margin-bottom:14px;"><h3>${proc.name}</h3>`;
        CATS.forEach(cat => {
    ...
    if (nonCircCount === 1 && raiseProcLabels.length === 1) {
        // Only one non-circling approach: Replace circling with raised result if needed
        ...
        // Show each raised circling if applicable
